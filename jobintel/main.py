...
@app.patch("/api/sources/{source_type}/{company}")
def update_source(source_type: str, company: str, update: SourceUpdate):
    try:
        return set_source_enabled(source_type, company, update.enabled)
...
<select id="pool"><option value="">All discovery pools</option><option value="benchmark">Scheduled-task benchmark</option><option value="standard">Standard discovery</option></select>
...
function render(){const e=document.querySelector('#eligibility').value,s=document.querySelector('#state').value,p=document.querySelector('#pool').value,q=document.querySelector('#search').value.toLowerCase();const jobs=allJobs.filter(j=>(!e||j.eligibility_status===e)&&(!s||j.review_state===s)&&(!p||(p==='benchmark'?j.benchmark_candidate:!j.benchmark_candidate))&&(!q||`${j.title} ${j.company}`.toLowerCase().includes(q)));document.querySelector('#summary').textContent=`${jobs.length} of ${allJobs.length} jobs · profile ${profileVersion}`;document.querySelector('#jobs').innerHTML=jobs.length?jobs.map(card).join(''):'<div class="empty">No jobs match these filters.</div>'}
function card(j){const eligibilityReasons=j.eligibility_reasons.map(r=>`<li><strong>${esc(r.explanation)}</strong>${r.evidence?`<div class="reason">Evidence: ${esc(r.evidence)}</div>`:''}</li>`).join(''),signals=(j.benchmark_fit_signals||[]).map(v=>`<li>${esc(v)}</li>`).join(''),concerns=(j.benchmark_concerns||[]).map(v=>`<li>${esc(v)}</li>`).join(''),benchmark=j.benchmark_candidate?`<details open><summary>Scheduled-task selection rationale</summary><p>${esc(j.benchmark_rationale)}</p>${signals?`<strong>Fit signals</strong><ul>${signals}</ul>`:''}${concerns?`<strong>Concerns</strong><ul>${concerns}</ul>`:''}<div class="meta">${esc(j.benchmark_task_name||j.benchmark_task_id)} · reported ${esc(j.benchmark_reported_at)}</div></details>`:'';return `<article class="job" data-id="${j.database_id}"><h2>${j.canonical_url?`<a href="${esc(j.canonical_url)}" target="_blank" rel="noopener">${esc(j.title)}</a>`:esc(j.title)}</h2><div class="meta">${esc(j.company)} · ${esc(j.location||'Location not stated')} · ${esc(j.source)}</div><div class="badges"><span class="badge ${j.eligibility_status}">${esc(j.eligibility_status)}</span><span class="badge">${esc(j.review_state)}</span>${j.benchmark_candidate?'<span class="badge benchmark">benchmark candidate</span>':''}${j.match_label?`<span class="badge">${esc(j.match_label)}</span>`:''}</div>${benchmark}<details><summary>Eligibility explanation</summary><ul>${eligibilityReasons}</ul></details><details><summary>Posting text</summary><p>${esc(j.description).replace(/\n/g,'<br>')}</p></details><div class="review"><select class="review-state" aria-label="Review state">${options(states,j.review_state)}</select><select class="match-label" aria-label="Match label">${options(labels,j.match_label||'')}</select><select class="review-reasons" multiple aria-label="Interest or rejection reasons">${reasonOptions(j.reason_codes)}</select><textarea class="notes" placeholder="Notes">${esc(j.notes)}</textarea><button onclick="save(${j.database_id})">Save review</button></div></article>`}
...
['eligibility','state','pool','search'].forEach(id=>document.querySelector('#'+id).addEventListener('input',render));document.querySelector('#reload').addEventListener('click',load);load().catch(error=>document.querySelector('#summary').textContent=`Unable to load: ${error.message}`);
document.querySelector('#manage-sources').addEventListener('click',()=>{const panel=document.querySelector('#sources-panel');panel.hidden=!panel.hidden;if(!panel.hidden)loadSources()});
</script></body></html>'''
